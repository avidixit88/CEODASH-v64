...
SEARCH_FALLBACK_TIMEOUT_SECONDS = 1.25
...
def _expand_validated_company_urls(source: SponsorEvidenceSource, seed_url: str) -> list[tuple[int, str]]:
    """After validating a canonical company domain, try common evidence paths.

    Common paths/subdomains are adaptive infrastructure guesses, not sponsor
    hardcoding. Each URL is still page-validated before it can be parsed.
    """
    ...
    # Try common evidence subdomains for the same registrable domain.
    for sub in COMMON_COMPANY_EVIDENCE_SUBDOMAINS:
        for path in ("/news-releases", "/news", "/press-releases", "/events-presentations"):
    ...
    for url in dict.fromkeys(candidates):
        ...
        if len(out) >= MAX_COMPANY_SITE_CANDIDATE_URLS:
            break
    return out
    ...
    if len(scored_urls) < MAX_COMPANY_SITE_CANDIDATE_URLS:
        for guessed in _sponsor_domain_guesses(source):
            ...
            if ok:
                ...
                if page_ok:
                    scored_urls.append((score + page_score, guessed))
...
def build_sponsor_evidence_summary(
    ...
    for source in fast_screen_budget_slice:
        ...
        # v0.9.47: route isolation. A throttled/500 RSS/news route must not
        # prevent the sponsor-owned company/newsroom route from running. The
        # previous all-in-one try block meant a transient fast-screen failure
        # could silently skip the very fallback we needed for high-priority
        # sponsors such as dynamically discovered NextCure/SIM0505.
        try:
            raw_screen_items.extend(_fast_screen_news_items(source))
        except Exception as exc:
            if is_focus_source:
                focus_trace_errors.append(f"fast news screen failed; continuing to company-site route: {type(exc).__name__}: {exc}")
            if len(errors) < 12:
                errors.append(f"fast news screen {source.sponsor}: {type(exc).__name__}: {exc}")

        # For sponsor-grade companies, add a generic company-site route. This
        # intentionally includes private and non-IR sponsors: news, press,
        # media, events, science, publications, and pipeline pages are eligible.
        # It runs even when RSS/news metadata fails.
        if len(fast_screened_names) <= MAX_COMPANY_SITE_SCREEN_SPONSORS and _entity_grade(source) in {"public_company", "sponsor_grade_company", "possible_company"}:
            ...
            try:
                ...
                if is_focus_source:
                    focus_trace_errors.append(f"company-site screen failed after fallback attempt: {type(exc).__name__}: {exc}")
                if len(errors) < 12:
